...
export const DENY_ERROR = Object.freeze({
...
export const DOES_NOT_PROMISE = 'a grant does not guarantee execution (CAS may still fail)';

export const ARGS_SHAPE = Object.freeze({
...
export interface DenyRemedy {
  ...
  action_required: { tool: 'preflight_change_set'; mode: 'authorize'; args_shape: Record<string, string> };
 ...
 * @returns {object|null}
 */
export function buildDenyRemedy(
  { error, target = null, fingerprint = null, observed = null }: {
  ...
  if (!error || !(Object.values(DENY_ERROR) as string[]).includes(error)) return null;

  const remedy: DenyRemedy = {
    error,
 ...
 */
export function denyErrorForReason(reason: unknown): DenyError | null {
